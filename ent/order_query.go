...
func (oq *OrderQuery) QueryCart() *CartQuery {
	query := (&CartClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, order.CartTable, order.CartColumn),
		)
...
func (oq *OrderQuery) WithCart(opts ...func(*CartQuery)) *OrderQuery {
	query := (&CartClient{config: oq.config}).Query()
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withCart; query != nil {
		if err := oq.loadCart(ctx, query, nodes,
			func(n *Order) { n.Edges.Cart = []*Cart{} },
			func(n *Order, e *Cart) { n.Edges.Cart = append(n.Edges.Cart, e) }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) loadCart(ctx context.Context, query *CartQuery, nodes []*Order, init func(*Order), assign func(*Order, *Cart)) error {
	...
	query.Where(predicate.Cart(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.CartColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.order_cart
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_cart" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_cart" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
