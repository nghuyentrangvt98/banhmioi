...
func (cq *CartQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, cart.UserTable, cart.UserColumn),
		)
...
func (cq *CartQuery) QueryProduct() *ProductQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, cart.ProductTable, cart.ProductColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CartQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Cart, error) {
	...
	if cq.withUser != nil || cq.withProduct != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withProduct; query != nil {
		if err := cq.loadProduct(ctx, query, nodes, nil,
			func(n *Cart, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CartQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *User)) error {
	...
	for i := range nodes {
		if nodes[i].cart_user == nil {
			continue
		}
		fk := *nodes[i].cart_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cart_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CartQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *Product)) error {
	...
	nodeids := make(map[int][]*Cart)
	for i := range nodes {
		if nodes[i].cart_product == nil {
			continue
		}
		fk := *nodes[i].cart_product
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cart_product" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
